...
export default Calendar = function ({ route, navigation }) {
  ...
  const getData = () => {
    try {
      SportSchedRepo.getSportSchedByUserID(customerID)
        .then((result) => {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate("FeedbackDetail", { sched: sched, info: info, id: item.id })}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        onPress={() => navigation.navigate("Add Calendar", { sched: sched, customerID: customerID })}
